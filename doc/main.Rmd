...
```{r, Feature and Label Construction}
...
for (i in 1:length(file_name_vec)){
  ...
  # generate features for every token in the current file
  current_feat <- as.data.frame(t(sapply(tesseract_vec, extractFeature)))
  # generate labels for every token in the current file
  current_label <- unlist(sapply(1:length(current_tesseract_txt), extractLabel,
                         current_tesseract_txt,current_ground_truth_txt))
  # generate data frame consists of (word, filenum, index of word, label)
  current_dat <- data.frame(word = tesseract_vec,
                  filenum = i,
                  index = index(tesseract_vec),
                  label = current_label)
  # merge the feature and label data frame with previous ones
  if (i == 1) {
    label_dat <- current_dat
    feature_dat <- current_feat
  } else {
    label_dat <- rbind(label_dat, current_dat)
    feature_dat <- rbind(feature_dat, current_feat)
  }
  # print message
  if (i %% 10 == 0) {cat(i, "files finished\n")}
}
cat("------------Feature and Label Construction Done------------\n")


save(feature_dat, file = "../output/feature_dat.RData")
save(label_dat, file = "../output/label_dat.RData")
```

* ###Cross Validation

```{r}
load("../output/feature_dat.RData")
load("../output/label_dat.RData")
cv_ind <- sample(1:nrow(feature_dat), 10000, replace = FALSE)
cv_feature_dat <- feature_dat[cv_ind, ]
cv_label_dat <- label_dat[cv_ind, ]
cv <- tune(svm, train.x=cv_feature_dat, train.y=cv_label_dat$label, 
