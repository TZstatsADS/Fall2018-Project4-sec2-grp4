...
Here we cross validate $C$ and $\gamma$ and train the SVM model with each text file group.
...
dat$label <- as.factor(as.character(dat$label))
...
#Saving the outputs
svm_prediction <- rbind(matrix(a$pred),matrix(b$pred),
                        matrix(c$pred),matrix(d$pred), 
                        matrix(e$pred))
prediction_list <- list(svm_prediction)

# Saving the results
save(svm_prediction, file = "../output/SvmLabels.RData")
save(prediction_list, file = "../output/SvmLabels.RData")
...
```{r}
load("../output/accuracy.RData")
load("../output/listtempSvmLabels.RData")
load("../output/tempSvmLabels.RData")
```
