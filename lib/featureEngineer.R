...
BigramList <- fromJSON(file = "../data/bigrams.json")
Bigram <- as.data.frame(matrix(unlist(BigramList),ncol = 2, byrow = T))
colnames(Bigram) <- c("bigram","freq")
Bigram$freq <- as.numeric(as.character(Bigram$freq))
save(Bigram,file = "../Bigram.RData")
getBigr <- function(token_bigram, c=10000) {
